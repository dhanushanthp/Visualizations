...
(function(undefined) {
	 ...
	 * GEXF Library =============
	 * 
	 * Author: PLIQUE Guillaume (Yomguithereal) URL:
	 * https://github.com/Yomguithereal/gexf-parser Version: 0.1.1
	 */
	...
	var _helpers = {
		getModelTags : function(xml) {
			var attributesTags = xml.getElementsByTagName('attributes'), modelTags = {}, l = attributesTags.length, i;

			for (i = 0; i < l; i++)
				modelTags[attributesTags[i].getAttribute('class')] = attributesTags[i].childNodes;
		...
		getRGB : function(values) {
			return (values[3]) ? 'rgba(' + values.join(',') + ')' : 'rgb('
	...
	/**
	 * Node structure. A function returning an object guarded with default
	 * value.
	 * 
	...
	/**
	 * Edge structure. A function returning an object guarded with default
	 * value.
	 * 
	...
	/**
	 * Graph parser. This structure parse a gexf string and return an object
	 * containing the parsed graph.
	 * 
	...
	function Graph(xml) {
		...
		function _metaData() {
			...
			// Last modified date
			metas.lastmodifieddate = _xml.els.meta
					.getAttribute('lastmodifieddate');
		...
		function _data(model, node_or_edge) {
			...
			model.map(function(a) {
				...
				// Default value?
				data[a.id] = !(a.id in ah) && 'defaultValue' in a ? _helpers
						.enforceType(a.type, a.defaultValue) : _helpers
						.enforceType(a.type, ah[a.id]);
		...
		function _nodes(model) {
			...
			_helpers.nodeListEach(_xml.els.nodes, function(n) {
				...
				for ( var i = 0; i < x.length; i++) {
					if (x[i].getAttribute('for') == 'i_con_title'
							|| x[i].getAttribute('for') == 'i_phrase_name'
							|| x[i].getAttribute('for') == 'pg_no') {
		...
		function _nodeViz(node) {
			var x = node.getElementsByTagName('attvalue');
			var viz = {};

			// Color
			var color_el = _helpers
					.getFirstElementByTagNS(node, 'viz', 'color');
			...
			if (color_el) {
				var color = [ 'r', 'g', 'b', 'a' ].map(function(c) {
					...
					for ( var i = 0; i < x.length; i++) {
						...
						if (x[i].getAttribute('for') == 'i_phrase_name') {
							if (c == 'r') {
								return 51;
							} else if (c == 'g') {
								return 153;
			...
			// Position
			var pos_el = _helpers.getFirstElementByTagNS(node, 'viz',
					'position');
			...
			// Shape
			var shape_el = _helpers
					.getFirstElementByTagNS(node, 'viz', 'shape');
			if (shape_el)
		...
		function _edgeViz(edge) {
			...
			// Color
			var color_el = _helpers
					.getFirstElementByTagNS(edge, 'viz', 'color');
			...
			// Shape
			var shape_el = _helpers
					.getFirstElementByTagNS(edge, 'viz', 'shape');
			if (shape_el)
				viz.shape = shape_el.getAttribute('value');

			// Thickness
			var thick_el = _helpers.getFirstElementByTagNS(edge, 'viz',
					'thickness');
			if (thick_el)
		...
		// Returning the Graph
		// ---------------------
		var nodeModel = _model('node'), edgeModel = _model('edge');
	...
	function fetch(gexf_url, callback) {
		var xhr = (function() {
			...
			if (window.ActiveXObject) {
				names = [ 'Msxml2.XMLHTTP.6.0', 'Msxml2.XMLHTTP.3.0',
		...
		// Async?
		var async = (typeof callback === 'function'), getResult;
